...
for(d in 1:length(data$s)){
  ...
  for (i in 1:20){
    ...
    LEARN <- function(params) {
      ...
      LL<-  for(t in 1:length(fdk_data)) {
        ...
        if ( rep_data[t] == "Nice80") {
          ...
          K_n[1,t]= params['w_n'][[1]]*A_m8 +(1-params['w_n'][[1]])
          alpha_n[1,t] = params['k_n'][[1]]*K_n[1,t]
          ...
          #step one of calculating learning rate as constant or dynamic
          K_p[1,t]= params['w_p'][[1]]*A_m6 +(1-params['w_p'][[1]])
          alpha_p[1,t] = params['k_p'][[1]]*K_p[1,t]
          
          K_n[1,t]= params['w_n'][[1]]*A_m6 +(1-params['w_n'][[1]])
          alpha_n[1,t] = params['k_n'][[1]]*K_n[1,t]
          ...
          #step one of calculating learning rate as constant or dynamic
          K_p[1,t]= params['w_p'][[1]]*A_n6 +(1-params['w_p'][[1]])
          alpha_p[1,t] = params['k_p'][[1]]*K_p[1,t]
      ...
      Llik =-sum(log(chosen_prob), log(prior_w_p), log(prior_k_p),log(prior_l_p), log(prior_w_n), log(prior_k_n),log(prior_l_n),log(prior_tau), .Machine$double.eps)
      return(Llik)
      plot(Llik)
    ...
    RW_1 <- optim(par      = unlist(params),   # Initial guess for beta
                  ...
                  control = list(pgtol= TRUE),# sets convergence tolerance 
...
write.csv(par_data, "/Users/tuk38887/Desktop/BehavData/Analyses/ModelFitting/M10_AssV_fdk_MAP_2.0.csv", row.names = F)
