...
for(d in 1:length(data$s)){
  ...
  for (i in 1:20){
    ...
    params$w = runif(1, min=0.01, max=.99)
    params$k = runif(1, min=0.01, max=.99)
    params$l = runif(1, min=0.01, max=.99)
    

    #resp_prob empty matrices
    pred_prob = matrix(ncol = length(x[,1]), nrow=1)
    K= matrix(ncol = length(x[,1]), nrow=1)
    #makes empty dataframe for learning rate values for each trial
    alpha= matrix(ncol = length(x[,1]), nrow=1)
    #makes empty dataframe for learning rate values for each trial
    ...
    LEARN <- function(params) {
      ...
      #LL<-  for(t in 1:length(fdk_data)) {
       #t=2
        ...
        if ( rep_data[t] == "Nice80") {
          ...
          K[1,t]= params['w'][[1]]*A_n8 +(1-params['w'][[1]])
          alpha[1,t] = params['k'][[1]]*K[1,t]
          ...
          if (t==length(fdk_data)) {
            ...
            #update associability
            A_n8 = params['l'][[1]]*A_n8
            A_n8 = A_n8 + (1-params['l'][[1]])*(delta)^2
          }
          
        } else if ( rep_data[t] == "Mean80") {
          #step one of calculating learning rate as constant or dynamic
          K[1,t]= params['w'][[1]]*A_m8+(1-params['w'][[1]])
          alpha[1,t] = params['k'][[1]]*K[1,t]
          #####softmax function for choice 1 & 2 probabilities######
          #calculates choice probability for selecting mean (1) for trial t
          ...
          if (t==length(fdk_data)) {
            ...
            #update associability
            A_m8 = params['l'][[1]]*A_m8
            A_m8 = A_m8 + (1-params['l'][[1]])*(delta)^2
          }
  
        } else if ( rep_data[t] == "Mean60") {
          #step one of calculating learning rate as constant or dynamic
          K[1,t]= params['w'][[1]]*A_m6 +(1-params['w'][[1]])
          alpha[1,t] = params['k'][[1]]*K[1,t]
          #####softmax function for choice 1 & 2 probabilities######
          ...
          if (t==length(fdk_data)) {
            ...
            #update associability
            A_m6 = params['l'][[1]]*A_m6
            A_m6 = A_m6 + (1-params['l'][[1]])*(delta)^2
          }
          
        } else  {
          #step one of calculating learning rate as constant or dynamic
          K[1,t]= params['w'][[1]]*A_n6+(1-params['w'][[1]])
          alpha[1,t] = params['k'][[1]]*K[1,t]
          #####softmax function for choice 1 & 2 probabilities######
          #calculates choice probability for selecting mean (1) for trial t
          pred_prob[t] = softmax_pred(ExPred_n6, params['tau'][[1]])
          ...
          if (t==length(fdk_data)) {
            ...
            #update associability
            A_n6 = params['l'][[1]]*A_n6
            A_n6 = A_n6 + (1-params['l'][[1]])*(delta)^2
          }
        ...
        prior_tau <-dgamma(params['tau'][[1]], 1.2, scale = 1.5, log = FALSE)
         prior_w <-dbeta(params['w'][[1]], 1.1, 1.1, ncp = 0, log = FALSE)
         prior_k <-dbeta(params['k'][[1]], 1.1, 1.1, ncp = 0, log = FALSE)
         prior_l <-dbeta(params['l'][[1]], 1.1, 1.1, ncp = 0, log = FALSE)
        #prior_w = dnorm(params['w'][[1]], mean= 0, sd= 6.25)
        #prior_k = dnorm(params['k'][[1]], mean= 0, sd= 6.25)
        #prior_l = dnorm(params['l'][[1]], mean= 0, sd= 6.25)
        ...
        #            EV      = chosen_prob[t],
        #            Pr      = pred_prob[,t],
        #            Prediction  = paste(1:2),
        #            detla = delta,
        #            pred_real = pred_data[t],
        #            alpha  = alpha[t],
        #            A = A[t],
        #            K = K[t],
        #            Outcome = fdk_data[t])
     ...
     #write.csv(sim_dat, paste("/Users/tuk38887/Desktop/BehavData/Analyses/ModelFitting/M11/M11_",sub,"_Value_Sim.csv", sep = ""), row.names = F)
      ...
      Llik =-sum(log(chosen_prob), log(prior_w), log(prior_k),log(prior_l), log(prior_tau), .Machine$double.eps)
      return(Llik)
      plot(Llik)
...
write.csv(par_data, "/Users/tuk38887/Desktop/BehavData/Analyses/ModelFitting/M6.1LR_AssV_MAP_7.0.csv", row.names = F)
