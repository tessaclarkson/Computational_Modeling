...
for(d in 1:length(data$s)){
  ...
  for (i in 1:20){
    ...
    #   num = exp((ExPred*params))
    #   #denominator exp of all choices
    #   dem = exp((ExPred*params)) + exp(((1-ExPred)*params))
    #   #probability
    #   pred_prob = num/dem
    #   return (pred_prob)
    # }
    ...
    #change to multiply with params
    ...
    LEARN <- function(params) {
      ...
      Llik =-sum(log(chosen_prob), log(prior_LR_p), log(prior_LR_n), log(prior_tau), .Machine$double.eps)
      return(Llik)
      plot(Llik)
...
write.csv(par_data, "/Users/tuk38887/Desktop/BehavData/Analyses/ModelFitting/M4.2LR_fdk_MAP_7.0.csv")
