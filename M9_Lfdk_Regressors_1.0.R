...
#removes response column and then filter rest to remove NA values for no prediction trials
data <- na.omit(data[ -c(6) ])
...
for(d in 1:length(data$s)){
  ...
  params$tau =0.520372001
  params$w_m8 = 0.522454371
  params$w_n8 = 0.513904437
  params$w_m6 = 0.458396688
  params$w_n6 = 0.5117394
  params$k = 0.396464571
  params$l_p = 0.5097041
  params$l_n = 0.510838054
...
write.csv(all_sim_data,file=(paste("/Users/tuk38887/Desktop/BehavData/Analyses/Model_Validation/M9/M9_Lfdk_Regressors_1.0.csv",sep="")),row.names=FALSE)
