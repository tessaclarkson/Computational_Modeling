    ...
    LEARN <- function(params) {
      ...
      LL<-  for(t in 1:length(fdk_data)) {
        ...
        if ( rep_data[t] == "Nice80") {
          ...
          K_n8[1,t]= params['w'][[1]]*A_n8+((1-params['w'][[1]])*Fix)
          ...
          #step one of calculating learning rate as constant or dynamic
          K_m8[1,t]= params['w'][[1]]*A_m8+((1-params['w'][[1]])*Fix)
          ...
          #step one of calculating learning rate as constant or dynamic
          K_m6[1,t]= params['w'][[1]]*A_m6+((1-params['w'][[1]])*Fix)
          ...
          #step one of calculating learning rate as constant or dynamic
          K_n6[1,t]= params['w'][[1]]*A_n6 +((1-params['w'][[1]])*Fix)
      ...
      Llik =-sum(log(chosen_prob), log(prior_w), log(prior_k), 
                 log(prior_l_p),log(prior_l_n),
                 log(prior_tau), .Machine$double.eps)
      return(Llik)
      plot(Llik)
...
write.csv(par_data, "/Users/tuk38887/Desktop/BehavData/Analyses/ModelFitting/M9_AssV_Pred_Val_MAP_1wk_Lfdk_Decay.csv", row.names = F)
