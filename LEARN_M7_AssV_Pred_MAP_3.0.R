...
for(d in 1:length(data$s)){
  ...
  for (i in 1:20){
    ...
    params$w_p = runif(1, min=0.01, max=.99)
    params$k_p = runif(1, min=0.01, max=.99)
    params$l_p = runif(1, min=0.01, max=.99)
    params$w_u = runif(1, min=0.01, max=.99)
    params$k_u = runif(1, min=0.01, max=.99)
    params$l_u = runif(1, min=0.01, max=.99)
    
    
    #resp_prob empty matrices
    pred_prob = matrix(ncol = length(x[,1]), nrow=1)
    K_p= matrix(ncol = length(x[,1]), nrow=1)
    K_u= matrix(ncol = length(x[,1]), nrow=1)
    
    #makes empty dataframe for learning rate values for each trial
    alpha_p= matrix(ncol = length(x[,1]), nrow=1)
    alpha_u= matrix(ncol = length(x[,1]), nrow=1)
    ...
    LEARN <- function(params) {
      ...
      LL<-  for(t in 1:length(fdk_data)) {
        ...
        if ( rep_data[t] == "Nice80") {
          ...
          K_p[1,t]= params['w_p'][[1]]*A_n8 +(1-params['w_p'][[1]])
          alpha_p[1,t] = params['k_p'][[1]]*K_p[1,t]
          ...
          #step one of calculating learning rate as constant or dynamic
          K_p[1,t]= params['w_p'][[1]]*A_m8 +(1-params['w_p'][[1]])
          alpha_p[1,t] = params['k_p'][[1]]*K_p[1,t]
          #####softmax function for choice 1 & 2 probabilities######
          #calculates choice probability for selecting mean (1) for trial t
          ...
          #step one of calculating learning rate as constant or dynamic
          K_u[1,t]= params['w_u'][[1]]*A_m6 + (1-params['w_u'][[1]])
          alpha_u[1,t] = params['k_u'][[1]]*K_u[1,t]
          #####softmax function for choice 1 & 2 probabilities######
          #calculates choice probability for selecting mean (1) for trial t
          ...
          if (t==length(fdk_data)) {
            ...
            #update associability
            A_m6 = params['l_u'][[1]]*A_m6
            A_m6 = A_m6 + (1-params['l_u'][[1]])*(delta)^2
          }
          
        } else  {
          #step one of calculating learning rate as constant or dynamic
          K_u[1,t]= params['w_u'][[1]]*A_n6 + (1-params['w_u'][[1]])
          alpha_u[1,t] = params['k_u'][[1]]*K_u[1,t]
          #####softmax function for choice 1 & 2 probabilities######
          #calculates choice probability for selecting mean (1) for trial t
          ...
          if (t==length(fdk_data)) {
            ...
            #update associability
            A_n6 = params['l_u'][[1]]*A_n6
            A_n6 = A_n6 + (1-params['l_u'][[1]])*(delta)^2
          }
        ...
        prior_w_p = dbeta(params['w_p'][[1]], 1.1, 1.1, ncp = 0, log = FALSE)
        prior_k_p = dbeta(params['k_p'][[1]], 1.1, 1.1, ncp = 0, log = FALSE)
        prior_l_p = dbeta(params['l_p'][[1]], 1.1, 1.1, ncp = 0, log = FALSE)
        prior_w_u = dbeta(params['w_u'][[1]], 1.1, 1.1, ncp = 0, log = FALSE)
        prior_k_u = dbeta(params['k_u'][[1]], 1.1, 1.1, ncp = 0, log = FALSE)
        prior_l_u = dbeta(params['l_u'][[1]], 1.1, 1.1, ncp = 0, log = FALSE)
        # Save data
      ...
      Llik =-sum(log(chosen_prob), log(prior_w_p), log(prior_k_p),log(prior_l_p), log(prior_w_u), log(prior_k_u),log(prior_l_u),log(prior_tau), .Machine$double.eps)
      return(Llik)
      plot(Llik)
...
write.csv(par_data, "/Users/tuk38887/Desktop/BehavData/Analyses/ModelFitting/M7_AssV_Pred_MAP_5.0.csv", row.names = F)
