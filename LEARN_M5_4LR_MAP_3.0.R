...
for(d in 1:length(data$s)){
  ...
  for (i in 1:20){
    ...
    params = list()
    params$LR_m8 = runif(1, min=0.01, max=.99)
    params$LR_n8 = runif(1, min=0.01, max=.99)
    params$LR_m6 = runif(1, min=0.01, max=.99)
    params$LR_n6 = runif(1, min=0.01, max=.99)
    ...
    LEARN <- function(params) {
      ...
      LL<-  for(t in 1:length(fdk_data)) {
        ...
        if ( rep_data[t] == "Nice80") {
          ...
          delta <-  fdk_data[t] - ExPred_n8[pred_data[t]]
          update <- params['LR_n8'][[1]] * delta
          ExPred_n8[pred_data[t]] <- ExPred_n8[pred_data[t]] + update
        } 
          ...
          ####update function for expected value#####
          # compute the prediction error aka "delta"
          delta <-  fdk_data[t] - ExPred_m8[pred_data[t]]
          update <- params['LR_m8'][[1]] * delta
          ExPred_m8[pred_data[t]] <- ExPred_m8[pred_data[t]] + update
        } 
          ...
          ####update function for expected value#####
          # compute the prediction error aka "delta"
          delta <-  fdk_data[t] - ExPred_n6[pred_data[t]]
          update <- params['LR_n6'][[1]] * delta
          ExPred_n6[pred_data[t]] <- ExPred_n6[pred_data[t]] + update
          ...
          pred_prob[t] = softmax_pred(ExPred_m6, params['tau'][[1]])
                    ...
                    ####update function for expected value#####
          # compute the prediction error aka "delta"
          delta <-  fdk_data[t] - ExPred_m6[pred_data[t]]
          update <- params['LR_m6'][[1]] * delta
          ExPred_m6[pred_data[t]] <- ExPred_m6[pred_data[t]] + update
        } 
        ...
        prior_LR_m8 <-dbeta(params['LR_m8'][[1]], 1.1, 1.1, ncp = 0, log = FALSE)
        prior_LR_n8 <-dbeta(params['LR_n8'][[1]], 1.1, 1.1, ncp = 0, log = FALSE)
        prior_LR_m6 <-dbeta(params['LR_m6'][[1]], 1.1, 1.1, ncp = 0, log = FALSE)
        prior_LR_n6 <-dbeta(params['LR_n6'][[1]], 1.1, 1.1, ncp = 0, log = FALSE)
        prior_tau <-dgamma(params['tau'][[1]], 1.2, scale = 1.5, log = FALSE)
        
      }
      
      
      Llik =-sum(log(chosen_prob), log(prior_LR_m8), log(prior_LR_n8), log(prior_LR_m6), log(prior_LR_n6), log(prior_tau), .Machine$double.eps)
      return(Llik)
      plot(Llik)
...
write.csv(par_data, "/Users/tuk38887/Desktop/BehavData/Analyses/ModelFitting/M5.4LR_MAP_4.0.csv")
