...
for(d in 1:length(data$s)){
  ...
  for (i in 1:20){
    ...
    params = list()
    params$LR_p = runif(1, min=0.01, max=.99)
    params$LR_u = runif(1, min=0.01, max=.99)
    ...
    LEARN <- function(params) {
      ...
      LL<-  for(t in 1:length(fdk_data)) {
        ...
        if ( rep_data[t] == "Nice80") {
          ...
          update <- params['LR_u'][[1]] * delta
          ExPred_m8[pred_data[t]] <- ExPred_m6[pred_data[t]] + update
        } 
        ...
        prior_LR_p <-dbeta(params['LR_p'][[1]], 1.1, 1.1, ncp = 0, log = FALSE)
        prior_LR_u <-dbeta(params['LR_u'][[1]], 1.1, 1.1, ncp = 0, log = FALSE)
        prior_tau <-dgamma(params['tau'][[1]], 1.2, scale = 1.5, log = FALSE)
        
      }
      
      
      Llik =-sum(log(chosen_prob), log(prior_LR_p), log(prior_LR_u),log(prior_tau), .Machine$double.eps)
      return(Llik)
      plot(Llik)
...
write.csv(par_data, "/Users/tuk38887/Desktop/BehavData/Analyses/ModelFitting/M2.2LR_Pred_MAP_6.0.csv")
