...
for(d in 1:length(data$s)){
  ...
  for (i in 1:20){
    ...
    params = list()
    params$LR = runif(1, min=0.01, max=.99)
    ...
    LEARN <- function(params) {
      ...
      LL<-  for(t in 1:length(fdk_data)) {
        ...
        prior_LR <-dbeta(params['LR'][[1]], 1.1, 1.1, ncp = 0, log = FALSE)
        prior_tau <-dgamma(params['tau'][[1]], 1.2, scale = 1.5, log = FALSE)
        
      }
      
      
      Llik =-sum(log(chosen_prob), log(prior_LR), log(prior_tau), .Machine$double.eps)
      return(Llik)
      plot(Llik)
...
write.csv(par_data, "/Users/tuk38887/Desktop/BehavData/Analyses/ModelFitting/M1.1LR_MAP_4.0.csv")
